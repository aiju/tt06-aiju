/* this is based on a Z80 and modified for testing the 8080. probably still wrong in many places though. */
...
char *in_buf = " 20000\r\rY\r10 INPUT R\r20 PRINT 3.14159 * R * R\r30 END\rRUN\r 4\r";

static uint8_t
z80read(uint16_t addr)
{
	return memory[addr];
}

static void
z80write(uint16_t addr, uint8_t value)
{
	memory[addr] = value;
}
...
z80in(uint8_t addr)
{
	...
	case 0:
		return *in_buf == 0;
	case 1:
		if(*in_buf) return *in_buf++;
		return 0;
	case 0xff:
		return 0;
	default:
		printf("read from unknown I/O port %.2x\n", addr);
		return 0;
	}
}
...
ed(void)
{
	...
	printf("undefined z80 opcode ed%.2x at pc=%#.4x\n", op, scurpc);
	exit(1);
}
...
index(int n)
{
	...
	printf("undefined z80 opcode %.2x%.2x at pc=%#.4x\n", n ? 0xfd : 0xdd, op, scurpc);
	exit(1);
}

float
floatconvert(uint32_t fp)
{
	uint32_t mantissa = fp & ((1<<23)-1);
	uint32_t sign = fp >> 23 & 1;
	uint32_t exponent = fp >> 24;
	if(exponent > 0) exponent -= 2;
	union { uint32_t bits; float value; } f;
	f.bits = sign<<31 | exponent<<23 | mantissa;
	return f.value;
}
...
*/
	...
	printf("undefined z80 opcode %#.2x at pc=%#.4x\n", op, scurpc);
	exit(1);
}
